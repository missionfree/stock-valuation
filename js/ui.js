...
function getTempLabel(temp) {
  if (temp < 25) return { label: '冰点', color: '#00AA00' };
  if (temp < 40) return { label: '偏冷', color: '#44CC44' };
  if (temp < 55) return { label: '温和', color: '#FFAE00' };
  if (temp < 70) return { label: '偏热', color: '#FF6600' };
  if (temp < 85) return { label: '过热', color: '#FF3300' };
  return { label: '极热', color: '#FF0000' };
}
...
function renderPortfolioAnalysis() {
  ...
  if (hasSignals && data.total > 0) {
    ...
    html += '<span class="pa-legend-item"><span class="pa-legend-dot" style="background:#FF0000"></span>低位 ' + data.lowCount + '只</span>';
    html += '<span class="pa-legend-item"><span class="pa-legend-dot" style="background:#FFAE00"></span>中位 ' + data.midCount + '只</span>';
    html += '<span class="pa-legend-item"><span class="pa-legend-dot" style="background:#00AA00"></span>高位 ' + data.highCount + '只</span>';
    ...
    if (p.items.length === 0) {
      ...
      displayItems.forEach(function(item) {
        ...
        if (sig && sig.metrics) {
          ...
          var parts = [];
          if (m.position) {
            var posColor = m.position === '低位' ? 'rgba(255, 0, 0,0.7)' : (m.position === '高位' ? 'rgba(0, 170, 0,0.7)' : 'rgba(158,158,158,0.6)');
